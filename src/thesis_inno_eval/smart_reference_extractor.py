...
class SmartReferenceExtractor:
    ...
    def _get_reference_boundaries(self, text: str) -> Tuple[int, int]:
        ...
        # 查找参考文献结束位置
        # 修正边界问题：参考文献后通常是"攻读学位期间成果"，然后才是"致谢"
        end_markers = [
            '攻读硕士学位期间获得的成果',  # 最优先：标准的成果展示部分
            '攻读学位期间发表',          # 其次：发表论文部分
            '攻读硕士学位期间发表',      # 变体：硕士学位期间发表
            '攻读学位期间取得',          # 变体：取得成果
            '攻读学位期间的研究成果',    # 变体：研究成果
            '个人简历',                  # 个人简历部分
            '作者简介',                  # 作者简介部分
    ...
    def _extract_with_regex(self, ref_text: str) -> List[str]:
        ...
        # 保存用于调试
        self._last_ref_section = ref_text
        ...
        # 检查是否有无编号格式的匹配（模式5）
        has_no_number_refs = any(pattern_type == 4 for _, _, _, _, pattern_type in all_matches)
        
        for num, content, start, end, pattern_type in all_matches:
            # 对于无编号格式，放宽验证条件
            if pattern_type == 4:  # 无编号格式
                # 检查内容长度
                if len(content) < 15:
                    print(f"   ⚠️ 跳过过短内容（{len(content)} 字符）")
                    continue
                    
                # 清理内容
                clean_content = re.sub(r'\s+', ' ', content).strip()
                
                # 验证是否为有效参考文献（对无编号格式更宽松）
                if self._is_valid_reference_no_number(clean_content):
    ...
    def _is_valid_reference_no_number(self, line: str) -> bool:
        """检查无编号格式的参考文献是否有效"""
        # 基本长度检查
        if len(line) < 15:
            return False
        ...
        publication_keywords = [
            ...
            # 中文关键词
            '期刊', '会议', '学报', '大学学报', '论文集', '出版社', '学院学报',
            '计算机', '应用', '技术', '研究', '分析', '探讨', '论文',
            '硕士', '博士', '学位论文', '毕业论文', '年第', '期', '页',
            '编', '著', '主编', '总编', '卷', '册', '版', '[J]', '[D]', '[C]',
        ...
        # 检查是否包含页码、卷号等信息
        has_page_info = bool(re.search(r':\d+[-–—]\d+|第\d+期|\(\d+\)|vol\.|no\.|pp\.', line, re.IGNORECASE))
        
        # 对于无编号格式，只需要满足2个条件即可
        valid_conditions = [has_publication, has_year, has_author, has_page_info]
        return sum(valid_conditions) >= 2
